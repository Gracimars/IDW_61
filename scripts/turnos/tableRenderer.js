...
const getTurnos = () => {
  const storedData = localStorage.getItem("dbTurnos");
  const turnosData = storedData ? JSON.parse(storedData) : [];
  return turnosData;
};

const getMedicos = () => {
  return JSON.parse(localStorage.getItem("dbMedicos")) || [];
};
...
function renderTurnos() {
  ...
  if (!turnos) {
    console.log("Error al cargar los turnos.");
    return;
  }

  const container = document.getElementById("rowTurno");
  if (!container) {
  ...
  turnos.forEach((turno, index) => {
    ...
    const medicoNombre = medico ? `${medico.nombre} ${medico.apellido}` : "---";
    const especialidad = medico
      ? especialidades.find((esp) => esp.id === medico.especialidad)?.nombre ||
        "---"
      : "---";

    const disponibleBadge = turno.disponible
      ...
      <td class="text-center">
        <div class="d-flex gap-1 justify-content-center">
          <button 
            ...
            onClick="openEditModalTurno(${turno.id})">
            ${icons.edit}
          </button>
          <button 
            data-id="${turno.id}" 
...
function handleDeleteTurno(id) {
  ...
  const deleteModal = new bootstrap.Modal(
    document.getElementById("deleteModalTurno")
  );
  deleteModal.show();
}
