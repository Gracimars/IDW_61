...
const handleSubmit = async (event) => {
  ...
  const idMedico =
    foundMedico?.id ?? Math.max(...medicosDB.map((medico) => medico.id)) + 1;
  ...
  modalContent.innerHTML = `${
    foundMedico ? "Modificaste" : "Creaste"
  } al médico con éxito!`;
...
function renderObrasSocialesCheckboxes(selectedIds = []) {
  const obrasSociales =
    JSON.parse(localStorage.getItem("dbObrasSociales")) || [];
...
const formInitializer = () => {
  ...
  const especialidades =
    JSON.parse(localStorage.getItem("dbEspecialidades")) || [];
  ...
  const mappedOptions = especialidades
    .map(
      (esp) => `
    <option value="${esp.id}" ${
        esp.id === foundMedico?.especialidad ? "selected" : ""
      }>${esp.nombre}</option>`
    )
    .join("");
